...
_CAM_INTRINSICS, _, _ = bot.camera.get_intrinsics() # FIXME
...
if "tm700" in robot_name:
    ...
    def open_gripper(force=False):
        if force:
            execute_urp('open_force', sleep=0.9)
        else:
            execute_urp('open', sleep=0.9)
        start_remote_ROS_control()

    def close_gripper(depth_compensation=False, force=False):
        if depth_compensation:
            execute_urp('close_d', sleep=0.8)
        else:
            if force:
                execute_urp('close_force', sleep=0.8)
            else:
    ...
    while True:
        try:
            ...
            if line.startswith("get_image"):
                ...
                del params, xyz, rgb
            elif line.startswith("get_xyz_transform"):
                params = line.split()
                if len(params)==3: # get_xyz_transform target source
                    target = str(params[1].strip())
                    source = str(params[2].strip())
                    T, quat = get_xyz_transform(listener, target, source)
                    R = RotationS.from_quat(quat).as_dcm().astype(np.float64) # (3, 3)
                    T = np.asarray(T, dtype=np.float64).reshape(3, 1)
                    RT = np.append(R, T, axis=1) # (3, 4)
                    b64 = encode_numpy(RT)
                    msg(b64)
                else:
                    msg("Error: Wrong arguments in get_xyz_transform!")
                ...
                del params
            elif line.startswith("open_gripper_force"):
                msg("Opening gripper")
                open_gripper(force=True)
            ...
            elif line.startswith("close_gripper_force"):
                msg("Closing gripper")
                close_gripper(force=True)
