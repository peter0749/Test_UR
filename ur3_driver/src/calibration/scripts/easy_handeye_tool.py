...
################################
# Setup camera topic
camera_topic = rospy.get_param('table_camera_topic', '/camera/color/image_raw')
camera_topic_shelf = rospy.get_param('shelf_camera_topic', '/camera_shelf/color/image_raw')
img = None
img_lock = threading.Lock()
img_shelf = None
img_shelf_lock = threading.Lock()
...
def shelf_image_callback(data):
    rgb_img = CvBridge().imgmsg_to_cv2(data, "bgr8")
    img_shelf_lock.acquire()
    global img_shelf
    rv, buffer = cv2.imencode('.png', rgb_img)
    img_shelf = io.BytesIO(buffer)
    img_shelf_lock.release()

rospy.Subscriber(camera_topic, Image, image_callback, queue_size=1)
rospy.Subscriber(camera_topic_shelf, Image, shelf_image_callback, queue_size=1)
################################
# Setup client
...
@app.route('/table_mode')
def switch_to_table_mode():
    global record
    record = JointPositionsRecord(record_type='table')
    record.from_file()
    return redirect('/')

@app.route('/shelf_mode')
def switch_to_shelf_mode():
    global record
    record = JointPositionsRecord(record_type='shelf')
    record.from_file()
    return redirect('/')
...
@app.route('/image_shelf.png')
def get_image_shelf():

    # Get image from ros topic
    img_shelf_lock.acquire()
    rv = send_file(img_shelf, attachment_filename='image_shelf.png', mimetype='image/png')
    img_shelf_lock.release()
    return rv
